...
public:
    ...
    void edit_task(QString name, QString description, int userId);
    void archive_task(); // ?
    void unarchive_task(); // ?
    void remove_task();
    void set_deadline(); // ?
    ...
    int timeLeftToDeadline(QDateTime start_day, QDateTime final_day); // ? => age khoruji ==0 shod, task_status = true;
...
private:
    ...
    QDateTime start_day;
    QDateTime final_day;
    bool userDoneTask; // age deadline tamum beshe vli user hnuz task ro anjam ndade bashe QMessage bede
    bool task_status;
    int id;
    static int counter;
